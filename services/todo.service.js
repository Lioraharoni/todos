...
function query(filterBy = {}) {
    return storageService.query(TODO_KEY)
        .then(totalTodos => {
            ...
            let todos = totalTodos
            // todos = totalTodos.map(todo => ({...todo}))
            ...
            if(filterBy.displayAll)
            {
                if(filterBy.displayAll == 'active')
                    todos = todos.filter(todo => !todo.isDone)
                else if(filterBy.displayAll == 'done')
                    todos = todos.filter(todo => todo.isDone)
            }

            console.log("{ totalTodos, todos} = ", { totalTodos, todos});
            let doneTodos = totalTodos.filter(todo => todo.isDone)

            return { todos, totalTodosCount: totalTodos.length, doneTodosCount: doneTodos.length}
        })
}
